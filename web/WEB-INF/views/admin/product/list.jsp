...
<%
    ...
    if (!keyword.isEmpty()) {
        queryBuilder.append("&keyword=").append(URLEncoder.encode(keyword, "UTF-8"));
    }
    if (!createdFrom.isEmpty()) {
        queryBuilder.append("&createdFrom=").append(URLEncoder.encode(createdFrom, "UTF-8"));
    }
    if (!createdTo.isEmpty()) {
        queryBuilder.append("&createdTo=").append(URLEncoder.encode(createdTo, "UTF-8"));
    }
    if (!trangThai.isEmpty()) {
        queryBuilder.append("&trangThai=").append(URLEncoder.encode(trangThai, "UTF-8"));
    }
    if (!maDanhMuc.isEmpty()) {
        queryBuilder.append("&maDanhMuc=").append(URLEncoder.encode(maDanhMuc, "UTF-8"));
    }
    if (!maThuongHieu.isEmpty()) {
        queryBuilder.append("&maThuongHieu=").append(URLEncoder.encode(maThuongHieu, "UTF-8"));
    }
    if (!maDoiBong.isEmpty()) {
        queryBuilder.append("&maDoiBong=").append(URLEncoder.encode(maDoiBong, "UTF-8"));
    }
...
<html lang="vi">
    <head>
        <meta charset="UTF-8">
        <title>Danh sách sản phẩm .LKsport</title>
